...
const Hero = ({ defaultMovieId = "tt15398776" }) => { // Oppenheimer por defecto
  const { details, fetchById } = useMediaDetails();
  const navigate = useNavigate();

  useEffect(() => {
    fetchById(defaultMovieId);
  }, [defaultMovieId, fetchById]);

  if (!details) return null;

  return (
    <div className="relative h-[85vh] w-full overflow-hidden border-b border-gray-800">
      {/* Background Layer con Blur y Overlay */}
